<div>
        ...
        <div class="overflow-x-auto">
            <flux:table
                ...
                <flux:table.columns>
                    ...
                    @canany(['customer_categories:update', 'customer_categories:delete'])
                                ...
                                <flux:table.cell>
                                    <flux:dropdown>
                                        ...
                                        <flux:menu>
                                            ...
                                            @can('customer_categories:delete')
                                                <x-delete-modal id="delete-customer-cat-{{ $category->id }}"
                                                    action="delete({{ $category->id }})" requireSlide="true"
                                                    title="Hapus kategori?"
                                                    description="You are about to delete this customer category. This action cannot be reversed." />
                                            @endcan
                                        </flux:menu>
                                    </flux:dropdown>
                                </flux:table.cell>
    ...
    <flux:modal :closable="false" scroll="body" name="create-customer-category-modal" class="md:max-w-2xl !rounded-3xl">
        <form wire:submit="save">
            <flux:heading>{{ $categoryId ? 'Edit' : 'Create' }} Category</flux:heading>
            <flux:description>{{ $categoryId ? 'Anda akan mengedit data kategori' : 'Anda akan membuat kategori baru' }}
            </flux:description>
            ...
            <div
                ...
                <label
                    ...
                    <div class="flex-1">
                        <input wire:model="code" placeholder="e.g. ECER"
                ...
                <label
                    ...
                    <div class="flex-1">
                        <input wire:model="name" placeholder="e.g. Eceran"
                            class="w-full bg-transparent border-none outline-none focus:ring-0 text-[15px] text-zinc-700 dark:text-zinc-300 placeholder-zinc-400 px-0 py-2 text-right" />
                    </div>
                </label>
            ...
            <div class="mt-6 flex flex-col gap-2">
                ...
                <flux:button class="!rounded-full" x-on:click="$flux.modal('create-customer-category-modal').close()" variant="outline">Batal
                </flux:button>
            </div>
        </form>
    </flux:modal>
